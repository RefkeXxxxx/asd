...
client.on("ready", () => {
    ...
    client.user.setPresence({game : { name : `Parancsok: /parancsok`, type: "WATCHING"}, status: "dnd"})
})
...
client.on("guildMemberAdd", async member => {
    let welcomechannel = member.guild.channels.find(c => c.name === "『👋』dsadas")
    let embed = new Discord.RichEmbed()
    .setTitle("Csatlakozás:")
    .setColor("#ff2a00")
    .addField(`Üdv a szerveren ${member.user.username}, érezd jól magad!`, "_ _")
    .setTimestamp()
    .setFooter(`MátrixBOT - Csatlakozás`, client.user.avatarURL)
    welcomechannel.send(embed)
  })
client.on("guildMemberRemove", async member => {
    let leavechannel = member.guild.channels.find(c => c.name === "『👋』dsadsa")
    let embed = new Discord.RichEmbed()
    .setTitle("Kilépés:")
    .setColor("#ff2a00")
    .addField(`Viszlát ${member.user.username}!`, "_ _")
    .setTimestamp()
    .setFooter(`MátrixBOT - Kilépés`, client.user.avatarURL)
